...
Use for Meta/Facebook Graph API HTTP Request nodes, Insights queries, and Meta campaign metric normalization.

## Endpoint and Auth

- Endpoint format: `https://graph.facebook.com/vXX.0/{ad_account_id}/insights` (pin an explicit version).
- Account ID should come from a trusted upstream field, never hardcoded.
...
- Attribution lag: D-1 data may be empty or incomplete early in the morning. A common pattern is to try D-1 first and fall back to D-2; if a workflow has such a fallback, preserve its wiring when editing.
