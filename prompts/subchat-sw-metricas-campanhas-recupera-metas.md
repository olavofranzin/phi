...
**Por que acontece:** esse node lê um valor de outro node chamado **`Edit Fields`**, com esta linha de código:

```js
const meta_valor = $('Edit Fields').first().json.meta_valor || 0;
```

Só que o node `Edit Fields` **nem sempre roda**. O fluxo tem dois caminhos que chegam no mesmo lugar:
...
Existem dois jeitos simples (você escolhe o mais simples que funcionar, **e me explica antes de aplicar**):
