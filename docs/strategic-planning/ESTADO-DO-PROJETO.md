...
| Lote 1 (workflow WF-DOC-Telemetria-Diaria — Onboarding+Curador+Global) | **APROVADO Antigravity rodada 3 (2026-06-13) — smoke E2E real autorizado.** Histórico: smoke `a03` revelou regressão (133 linhas; multi-fan-in) → `a04` corrigiu topologia mas faltava `numberInputs` → `a05` (`dddb84f`) cirúrgico, pré-revisão Claude 115/115 PASS → ✅ **APROVADO Antigravity rodada 3** ("o `a05` fecha cirurgicamente a única falha bloqueante do `a04`; causa-raiz do smoke endereçada de forma estruturalmente correta e agora protegida por teste"). Workflow `VubalOUaoBteCyC6` continua `active: false`. **Smoke real autorizado** — 1ª execução: esperado 19 linhas; 2ª execução (mesmo dia): esperado 0 (idempotência); digest Telegram coerente em ambas. Sem smoke verde, T6 não fecha e workflow não publica. |
...
| 2026-06-05/13 | Telemetria Mínima Lote 1 — ciclo de correções | ✅ `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2 → smoke revelou regressão (multi-fan-in) → `a04` (`b579816`, adiciona Merge Pre-Calcular) → pré-revisão REJEITADO (falta `numberInputs: 7`) → `a05` (`dddb84f`) → pré-revisão Claude 115/115 PASS → ✅ **APROVADO Antigravity rodada 3 (2026-06-13)**. | **Smoke real (Olavo)**: 1ª execução esperada 19 linhas, 2ª 0; digest Telegram. Smoke verde → `active=true` → fechar T6 |
...
| T6 | **Métricas operacionais não rastreadas** — Onboarding em prod há 9 dias, zero visibilidade | Conversa 2026-06-04 | **Em mitigação final — só falta smoke real** | Ciclo completo: Lote 0 → `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2 → smoke 2026-06-13 REGRESSÃO (133 linhas; multi-fan-in) → `a04` (`b579816`) topologia OK mas pré-revisão REJEITA (falta `numberInputs: 7`) → `a05` (`dddb84f`) ✅ pré-revisão Claude 115/115 → ✅ **APROVADO Antigravity rodada 3 (2026-06-13)**. Padrão inegociável consolidado: (a) multi-fan-in pra Code/Function node EXIGE Merge antes; (b) Merge consolidador DEVE declarar `numberInputs` = nº de upstreams. Restam: **smoke real (Olavo)** → `active=true` → T6 fecha. G2/G3 pro Lote 2. |
