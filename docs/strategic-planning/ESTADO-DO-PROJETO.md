...
>
> **NAVEGAÇÃO.** A porta única de navegação é o
> [`MAPA-DE-DOCUMENTACAO.md`](MAPA-DE-DOCUMENTACAO.md) — diz **onde procurar** e
> **quando criar** cada tipo de documento. O **§7** dele é o índice estruturado
> de **PHI·Mídia** (o score de campanha) e da frente **Saúde Digital** (4 camadas,
> ADRs 21-27, loop O.D.A.E., workflows por camada, DBs/DDLs). Lista exaustiva de
> artefatos = **Catálogo** (Notion).
>
...
|---|---|---|---|
| **2026-08-01** | **Credencial Google Ads API no n8n** — destrava o Guardião da Métrica-Mãe (ADR-29: detecção via `primary_for_goal`/`conversion_action_category`), o Consultor de Plataforma (ADR-31 camadas 1/2) e a camada 2 do ADR-31. | Olavo configura a credencial Google Ads API no n8n. | Bloqueia detecção do Guardião + camadas 1/2 do Consultor |
| **2026-08-01** | **Client Knowledge Pack povoado** — dossiê por cliente (ICP/STP/JTBD, margem, ticket/LTV, regime de decisão) disperso/não-povoado (spec em `client-knowledge-pack.md`). | Consolidar o conteúdo no CKP. | Bloqueia E2 (Julgamento/Hipóteses) + tira o Planejador do `[HIPÓTESE]` |
| **2026-08-01** | **Guardião da Métrica-Mãe (ADR-29) — implementação determinística** | Construir Camadas 0+2 (portão na ingestão + monitor diário vs histórico BQ + tarefa/checklist ADR-22) — não depende de LLM. | Bloqueia a proteção de integridade da métrica-mãe em produção |
| **2026-08-01** | **Wiring n8n dos agentes de Planejamento** (Planejador + Consultor de Plataforma; specs validadas por skill) | Criar os nós compondo BLOCO COMUM + prompt + skill inline; depende de credencial Anthropic + Google Ads API + CKP. | Bloqueia a frente Planejamento em produção (design pronto) |
| **2026-08-01** | **E1 Otimização — ativar** (Maestro no rascunho, persistência Notion pronta, guardrails/prompts validados) | Confirmar binding da credencial Anthropic → smoke-test real da cadeia E1 → publicar/ativar o `WF-T28-Analise-Campaign`. Ver ADR-28 + snapshot 2026-08-01. | Bloqueia a Camada 2 de análise em produção (design pronto) |
| **2026-08-01** | **Log de Otimizações aberto mas nunca fechado** — verificado no n8n vivo: o `Pipeline_v2` (`Criar Log Otimizacoes`) só faz `create` (`Resultado`="Aguardando Verificação"); nenhum nó grava o desfecho (`Resultado` Melhora/Piora + `Aprendizado Consolidado`) no auto-close/fechamento. | Adicionar write-back no Auto-Close (ou via agente Insights & Aprendizado) que atualiza o Log com resultado + aprendizado. | Não bloqueia operação; **bloqueia o aprendizado contínuo** (Camada Learn) |
| 2026-07-19 → **2026-07-31** | **Credencial Claude/Anthropic no n8n (cross-cutting) — REBAIXADO.** A credencial **`Anthropic account`** (`YifaYCQuGWjdd1Oh`) **existe** no n8n (confirmado via `list_credentials` 2026-07-31); deixou de ser "não configurada". O framework está construído: Diagnóstico (Agente 3) **vive** + Maestro (E1) no rascunho. | Confirmar o **binding** da credencial nos nós `Message a model`/`Maestro` (editor) → **1 smoke-test real** da cadeia E1 → ativar. Ver ADR-28 + snapshot 2026-08-01 (§2). | Rebaixado: não mais "sem credencial" — pendente binding + smoke |
...
| 2026-07-01 → **2026-08-01** | ~~**L3.0 — bug no placeholder de flags (`numeric(null)→0`)**~~ | ✅ **Resolvido 2026-08-01 (E1).** O `Build Deterministic Flags` foi retrabalhado: `numeric` retorna `null` (não 0); + fix `cpa:0` (conversions=0 ⇒ CPA/ROAS indefinidos) + flag `zero_conversao_com_gasto` + guardrails 8/9. Ver ADR-28/29. | Resolvido |
| 2026-06-30 → **2026-08-01** | ~~**Credencial Claude/Anthropic no n8n** (linha antiga)~~ | ✅ **Superseded** pela linha `2026-07-19 → 2026-07-31` (credencial existe; pendente binding + smoke). Ver ADR-28. | Resolvido/superseded |
...
| 2026-07-01 | **🔴 PHI·Mídia Score — keystone comprometido (sub-chat dedicado)** | Auditoria P2 + leitura do JSON live (`docs/audits/PHI - Pipeline_v2.json`) revelou **3+ camadas de falha** no `Calcular e Persistir PHI Score`: **(C, crítico)** o MERGE filtra o raw por `WHERE r.execution_id = source_execution_id`, mas `Buscar ID de Sucesso Hoje` **nunca produz `source_execution_id`** → filtro vazio → **0 linhas → não recalcula → `phi_score_current` serve linha velha/congelada** (dívida A.7b não terminada; explica o "50" da CLI-4); **(A)** `es/rs/os` são constantes `50.0` + flags `rs_data_insufficient/os_data_unavailable` hardcoded FALSE; **(B)** `mas/tss/fis` colapsam pra 50 em dado ralo (degradação silenciosa, viola ADR-21); **(D)** FALLBACK-* (A.6), FQN de projeto inconsistente, `mas` não ramifica CPA/ROAS. **Tudo downstream (L3.0/framework/loop) consome esse score** → validar/consertar o PHI·Mídia é **pré-requisito**. Brief profundo: `docs/handoff/2026-07-01-saude-digital-phi-midia-score-analise-subchat-brief.md` (com kit de evidências BQ a rodar). | **Bloqueia framework §4 + confiabilidade da Camada 2** |
...
| T12 | **Fragmentação de branches** — 3+ branches `claude/*` divergentes (`agentic-agency-planning-KwJEw`, `lucid-tesla-ZWcbr`, `affectionate-davinci-Ey2oV`, branch do score) e `main` ~1 mês atrás. Históricos não-relacionados impedem merge git trivial. | 2026-07-19 → 2026-08-01 | Média | Risco de integração + bus-factor. Decidir mecânica de consolidação de árvore com Olavo. **2026-08-01:** conteúdo da branch `paid-media-planning-frameworks` (doc D1) trazido para a `agentic-agency-planning`; `CLAUDE.md` restaurado do `main`. **Plano (Opção B — KwJEw vira trunk):** `docs/strategic-planning/plano-consolidacao-branches.md` — merge de teste mostrou **só 7 conflitos reais** (docs); críticas a absorver: `saude-digital-phi-midia-score` (keystone) + `wonderful-hawking` (adapter T28). |
| T13 | **Credencial Claude/Anthropic — REBAIXADO 2026-07-31.** A credencial (`Anthropic account`, `YifaYCQuGWjdd1Oh`) **existe** no n8n; a Camada 2 não está mais bloqueada por ausência de credencial. Resta confirmar o binding nos nós + smoke-test. | 2026-07-19 → 2026-07-31 | Baixa | Confirmar binding + smoke da cadeia E1; ver snapshot 2026-08-01 (§2) e ADR-28. |
...
| v0.1.48 | 2026-07-05 | **Camada de Conhecimento K0–K2 EXECUTADOS + K3 em curso (aprovação Olavo "aprovado, siga" nas D1–D5).** **K0 parcial:** os 3 docs de conhecimento + 4 handoffs do keystone (report 02/07, consolidação 04/07, Codex a02, t28 a01) trazidos do branch do score p/ o branch principal como arquivos (`5994e4d`) — **merge git é impossível: históricos não-relacionados**; consolidação plena de árvore fica como operação dedicada (a tentativa externa `007be39` foi revertida). **K1:** `docs/conhecimento/benchmarks-canonicos.yaml` — tabela canônica c/ IDs estáveis `[BM-*]`, **5 arbitragens** (ARB-CVR-01 site×plataforma, ARB-ROAS-01 margem>absoluto, ARB-LTVCAC-01 headroom, ARB-KEYWORD-01 ~20-30 conv, ARB-ESCOPO-01 aplicabilidade), faixas qualitativas consolidadas, seed §7.6 do R2, limiares e-commerce do G2 (`OP-ECOMM-*`, evidência rebaixada B/D) + **headers de governança/precedência nos 3 docs** (R2 canônico > R1; Módulo 28 = protocolo). **K2:** criados e catalogados 2 DBs sob Gerenciamento de Documentos — `PHI - Fontes de Conhecimento` (page `47424c69…61ec`, ds `533a0d0c-244d-4100-9be2-68b65793ab11`; registry de sync snake_case: file_id, modificado_em_fonte×ingerido_em, hash_versao, status_ingestao) com **7 fontes registradas** (4 Google Docs + 3 repo-docs, com aplicabilidade e força de evidência), e `PHI - Conhecimento (Insights & Pesquisas)` (page `7c8e95df…c036`, ds `3a20121f-420d-4e61-bca5-ad9fcc3003e9`; evidência A–D idêntica ao Banco, relation→Fontes). Catálogo +2 (`394b65e5…87e0`, `394b65e5…6102`); Banco de Estratégias já estava catalogado (`37db65e5…fd50`, 12/06). **K3 em execução (2 agentes background):** K3a exporta Temas 01–27 do G1 → `docs/conhecimento/temas/` + prompt-library; K3b popula o Banco c/ ~15–25 estratégias de G2/G3 (**Status=Rascunho**, gate humano) + staging em git. **Trilha A:** checagem da 1ª rodada v1.2 agendada (~07:17 BRT, cron de sessão). Pendências novas: K4 fiação do Log (aguarda Trilha A estável), K5 watcher de sync, K6 Curador, consolidação de árvore dos branches (decisão de mecânica com Olavo). |
| v0.1.47 | 2026-07-05 | **Aberta frente Camada de Conhecimento (aprendizagem dos agentes) — strawman BRUTO-v0.1 + digests dos 7 docs + achados de produção.** Olavo pediu retomada estratégica: bases de conhecimento antes dos agentes (casos internos de todas as áreas + conhecimento externo de profissionais-referência + Google Docs vivos que re-alimentam a base a cada atualização). **Inventário: ~60% já existe.** (a) `PHI — Banco de Estratégias (Tráfego Pago)` **já criado** (ds `0c6f2e04-b7fe-4e09-92b5-189b716c1dc2`, page `d8b953c8…910c`) com schema excelente (Tese formato Tema 07, Nível de Evidência A–D c/ regra "D nunca sustenta [CERTEZA]", Pré-condições/Contraindicações, aging Data da Fonte/Reavaliar em, relation bidirecional c/ Log) — porém **1 registro-semente só** (12/06); etapa real = popular+catalogar+governar, não criar. (b) Log de Otimizações: schema rico (Hipótese/Critério de Sucesso/Resultado Real/Aprendizado Consolidado/Janela de Observação/Origem da Ação/Componente PHI já c/ MIV) — gap é **fiação** (automações preencherem os campos; Critério+Janela obrigatórios na abertura) + tenant_id + localização legada (sob "GESTÃO DE CRISE"). (c) Guia de Agentes §2/§8 já define a memória institucional — materializar, não inventar. **Digests dos 7 docs** (2 agentes de leitura; anexo `docs/handoff/2026-07-05-camada-conhecimento-digests-7-docs.md`): stack de 5 estratos — **G1 Google "Estudo de IA Cognitiva" (Temas 01–27, 314k chars, vivo) = camada de raciocínio e a origem da dependência quebrada do Módulo 28**; R2 `pesquisa-trafego-pago.md` = benchmark canônico (único c/ fonte+data+região inline + nota anti-variância p/ agentes); R1 Benchmarks 2026 = complementar (números cedem a R2); R3 Módulo 28 = protocolo de consumo (Bloco Comum 7 regras; **7 prompts vazios**); G2 Santana e-comm (vivo 04/07, KPI-cards c/ limiares) + G3 Sobral (vivo 04/07, ~10 testes **n=1** infoproduto) = matéria-prima do Banco; G4 comercial (domínio separado — NÃO contaminar retrieval de análise). **Contradições a arbitrar antes de qualquer agente consumir:** CVR site×plataforma (risco de diagnóstico invertido), LTV:CAC >5:1 (leituras opostas R1×R2), ROAS por margem×absoluto "≥5:1", janela keyword 20+×~30 conversões. **Strawman** `docs/strategic-planning/camada-conhecimento/BRUTO-v0.1-design.md`: 5 estratos + fluxo Curador, 2 DBs novos (`PHI — Fontes de Conhecimento` registry+sync; `PHI — Conhecimento (Insights & Pesquisas)`), tabela canônica de benchmarks (IDs `BM-*`, seed §7.6 do R2), spec de fiação do Log (§4), sync Google Docs v1 semi-automático→v2 automação c/ gate humano (§5), Curador de Conhecimento N3 (§6), **5 decisões red-line D1–D5** (merge de branches, DBs, formato benchmarks, curador único×separado, prioridade A×B), lotes K0–K6 + Trilha A. **Achados de produção (n8n):** rodadas 07:00 de **03/07 e 04/07 do Pipeline_v2 = `error`** (janela exata da regressão `4b723285` prevista no report do sub-chat); **v1.2 (`9a174e50`, publicado 04/07) roda pela 1ª vez em 05/07 07:00 BRT** — Trilha A começa aí (checklist V1–V3 no branch do score). **⚠️ Governança de branch:** durante a sessão o branch `agentic-agency-planning` foi force-pushed 2× (baseline "consolidada" `007be39` criada e depois revertida p/ linha original `4b1a098`); docs de conhecimento seguem só no branch do score — **D1 (consolidação) é decisão urgente**, trabalho simultâneo em 2 linhas já causou a regressão de 02/07. |
| v0.1.46 | 2026-07-01 | **Auditoria dos workflows Saúde Digital (gate P1→P2) + brief profundo do PHI·Mídia + enriquecimento do MAPA.** (1) **Priorização + briefs** por workflow (`docs/handoff/…workflows-priorizacao-revisao.md`, commit `7139e65`); ajustes pós-feedback (`fa51e93`): **GATE** explícito (Codex roda P1 Agregador → P2 Pipeline_v2 e **para** p/ Olavo avaliar o método antes do P3), **Passo 0** (organizar WFs em pastas por objeto; folders `Onboarding`/`Métricas`/`phi_production`/… já existem), e correção **Daily Entry → `sw metricas campanhas`** (`W571K320aqIHsdtH`, active, sub-WF chamado por `operador unico metricas` `cLcimNoefTOnVVbd` cron 04:00) → **red flag #2 rebaixado** (writer do raw não está desligado). (2) **Codex executou P1/P2 até o gate** e commitou report (`9bdefdd`, `docs/handoff/…codex-report.md`): P1 defeitos confirmados (MERGEs, cadeia morta, Clarity-in-loop, search-terms placeholder); P2 diagnóstico do score-50. **Bloqueios de tooling** documentados: MCP não expõe move/`parentFolderId` (Passo 0 não feito); **`update_workflow` não persiste nó grande** (retorna `appliedOperations` sem gravar) → **read-back obrigatório** vira regra. Produção intacta (`activeVersionId=15b91f10`); nada publicado/ativado. (3) **Brief profundo do score** (`docs/handoff/…phi-midia-score-analise-subchat-brief.md`, commit `27e4443`) a partir da **dissecação do JSON live** (`docs/audits/PHI - Pipeline_v2.json`, 56 nós): diagnóstico em 4 camadas — **(C, crítico)** o MERGE filtra o raw por `WHERE r.execution_id = source_execution_id`, mas `Buscar ID de Sucesso Hoje` **nunca produz `source_execution_id`** → filtro vazio → 0 linhas → **não recalcula → `phi_score_current` serve linha congelada** (dívida A.7b não terminada; explica o `phi_value=50` da CLI-4); **(A)** `es/rs/os` = constantes `50.0` + flags `*_insufficient` hardcoded FALSE; **(B)** `mas/tss/fis` colapsam pra 50 em dado ralo; **(D)** FALLBACK-*/A.6, FQN de projeto inconsistente, `mas` sem ramificar CPA/ROAS. (4) **Descoberta:** o **ADR-004** (`359b65e5-c72b-819c-981c-fc1eaf79555f`, Aceito) **já documentava** os 5 problemas (incl. es/rs/os placeholder + score não-atualizado) desde 2026-05-07 — meu dump confirma que seguem sem correção 2 meses depois. **Gap-vs-intencional respondido:** es/rs/os é **placeholder documentado "NÃO implementado"** (não decisão limpa de v1.1); PHI·Mídia é 1 dos **6 pilares** do PHI/IDS (ADR-21) — os outros 5 pilares também sem cálculo (mesma lacuna). (5) **MAPA §7 enriquecido** (`MAPA-DE-DOCUMENTACAO.md`) como índice estruturado de PHI·Mídia + Saúde Digital (hierarquia IDS⊃PHI·Mídia, ADRs 003/004/21/22-27, workflows por camada, DBs/DDLs) + este ESTADO ganha ponteiro de NAVEGAÇÃO no topo. **Próximo:** sub-chat dedicado ao score (kit de evidências BQ → decisão CPA-only vs polimorfismo, candidato a ADR-008) **antes** do framework §4 e do P3. |
