...
| Lote 1 (workflow WF-DOC-Telemetria-Diaria — Onboarding+Curador+Global) | **REGRESSÃO ARQUITETURAL detectada no smoke E2E (2026-06-13); fix em 2 rodadas.** Smoke do `a03` (workflow `VubalOUaoBteCyC6`, `active: false`): 1ª execução criou **133 linhas** (esperado 19; exatamente 7×19); 2ª criou outras **133** (esperado 0). Causa-raiz: 7 leituras Notion conectavam todas em `Calcular Metricas in[0]` — n8n dispara Code node 1× por upstream trigger. Cleanup: 266 linhas apagadas por Olavo. **`a04` Codex (`b579816`)**: adicionou `[Telemetria] Merge Pre-Calcular` (merge v3 append) com as 7 leituras em inputs distintos 0..6 → Calcular Metricas (17 nodes). Pré-revisão Claude `a04` (2026-06-13): topologia correta, metricCode/digestCode idênticos ao a03, sandbox==workflow, **mas REJEITADO** — falta `numberInputs: 7` no Merge (default n8n = 2, então só esperaria 2 das 7 leituras → reabriria idempotência quebrada + métricas Curador/Global zeradas). Confirmado na definição oficial do node n8n. **Brief Codex `a05` entregue** (`docs/handoff/2026-06-13-telemetria-lote1-codex-a05-brief.md`): adicionar `numberInputs: 7` + check ps1 + regenerar. Aguarda Codex `a05` → pré-revisão Claude → Antigravity rodada 3 → **smoke real (decisivo)** → publish. |
...
| 2026-06-05/13 | Telemetria Mínima Lote 1 — ciclo de correções | ✅ `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2 → **smoke 2026-06-13 REVELOU REGRESSÃO** (multi-fan-in; 133 em vez de 19) → cleanup → `a04` Codex (`b579816`, adiciona Merge Pre-Calcular) → **pré-revisão Claude `a04` REJEITADO** (falta `numberInputs: 7`; default n8n=2 reabriria o bug) → brief Codex `a05` entregue (`docs/handoff/2026-06-13-telemetria-lote1-codex-a05-brief.md`). | Codex `a05` → pré-revisão Claude → Antigravity rodada 3 → smoke real (decisivo) → publish |
...
| T6 | **Métricas operacionais não rastreadas** — Onboarding em prod há 9 dias, zero visibilidade | Conversa 2026-06-04 | **Em mitigação — fix do bug de smoke em 2 rodadas** | Ciclo: Lote 0 → `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2. **Smoke 2026-06-13: REGRESSÃO** (multi-fan-in dispara Code node 7× → 133 linhas; idempotência zero). Cleanup OK. **`a04` Codex (`b579816`)** adicionou Merge Pre-Calcular (7 leituras em inputs 0..6), mas **pré-revisão Claude REJEITOU** por falta de `numberInputs: 7` (Merge v3 default=2 só esperaria 2 leituras → reabriria o bug; confirmado na def oficial n8n). Brief Codex `a05` entregue (parâmetro + check ps1). Padrão inegociável NOVO: (a) multi-fan-in pra Code/Function node EXIGE Merge antes; (b) Merge consolidador DEVE declarar `numberInputs` = nº de upstreams. Restam: Codex `a05` → re-revisão → Antigravity rodada 3 → novo smoke → publish → T6 fecha. G2/G3 pro Lote 2. |
