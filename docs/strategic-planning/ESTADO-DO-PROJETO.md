...
| v0.1.33 | 2026-06-18 | **Criado `MAPA-DE-DOCUMENTACAO.md` (porta única de navegação + guia de decisão) + 2 âncoras achadas.** Olavo propôs um "índice de livro" de toda a documentação. Diagnóstico: o índice de artefatos já existe (Catálogo, vivo) — um índice estático seria a própria duplicação a evitar + envelheceria. O gap real era a **camada de navegação + decisão**. Criado `docs/strategic-planning/MAPA-DE-DOCUMENTACAO.md` (commit `1de0e9a`): porta única de entrada com (1) como usar, (2) portas/índices, (3) DBs canônicos, (4) **guia "quando criar QUAL documento"** (ADR/SOP/Aprendizado/Mudança de Escopo/strawman/handoff/Documento/auditoria/linha em DB), (5) fluxo tácito→oficial, (6) âncoras por área, (7) docs técnicos do Produto PHI, (8) convenções, (9) regra de manutenção (não lista artefatos — aponta pro Catálogo, pra não envelhecer). README aponta pra ele no topo. **Achado bônus:** as 2 âncoras "não-rastreáveis" (Priorização + Doc&Ferramentas) EXISTEM — estavam linkadas no hub Gerenciamento de Documentos, só não apareciam em busca semântica (ver §5, resolvido). **Pendente de aprovação MCP Notion:** espelhar o Mapa como página Notion sob o hub + catalogar as 2 âncoras + fechar a tarefa no DB Gestão de Projetos. |
