...
| Lote 1 (workflow WF-DOC-Telemetria-Diaria — Onboarding+Curador+Global) | **`a05` Codex (`dddb84f`) PASSOU pré-revisão Claude — pronto pra Antigravity rodada 3.** Histórico curto: smoke do `a03` (`6a9c745`) revelou regressão 2026-06-13 (133 linhas em vez de 19 — multi-fan-in dispara Code node 7×); `a04` (`b579816`) corrigiu topologia (Merge Pre-Calcular com 7 leituras em inputs 0..6) mas foi REJEITADO por faltar `numberInputs: 7` (default n8n v3 = 2 → só esperaria 2 das 7 leituras); `a05` (`dddb84f`) adicionou `numberInputs: 7` + check ps1 + regenerou exports. **Pré-revisão Claude 2026-06-13: 115/115 PASS** — `numberInputs === 7` ✓, inputs `[0,1,2,3,4,5,6]` distintos ✓, hashes `metricCode` (`85065f64…`) e `digestCode` (`d6d72424…`) idênticos ao a03, sandbox==workflow byte-a-byte (`42c08612…`), 17 nodes, 19 add(), sem BOM/secrets/mojibake, `active: false`. **Brief Antigravity rodada 3 entregue** (`docs/handoff/2026-06-13-telemetria-lote1-antigravity-rodada3-brief.md`). Aguarda Antigravity rodada 3 → **smoke real (decisivo)** → publish. |
...
| 2026-06-05/13 | Telemetria Mínima Lote 1 — ciclo de correções | ✅ `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2 → smoke revelou regressão (multi-fan-in) → `a04` (`b579816`, adiciona Merge Pre-Calcular) → pré-revisão REJEITADO (falta `numberInputs: 7`) → `a05` Codex (`dddb84f`, adiciona `numberInputs: 7` + check ps1) → ✅ **pré-revisão Claude `a05` PASS (115/115)** → brief Antigravity rodada 3 entregue. | Antigravity rodada 3 → smoke real (decisivo) → publish |
...
| T6 | **Métricas operacionais não rastreadas** — Onboarding em prod há 9 dias, zero visibilidade | Conversa 2026-06-04 | **Em mitigação — pronto pra Antigravity rodada 3 + smoke decisivo** | Ciclo: Lote 0 → `a01` → G1 fix → REJEITADO rodada 1 → `a03` (`6a9c745`) → APROVADO Antigravity rodada 2 → smoke 2026-06-13 REGRESSÃO (133 linhas; multi-fan-in) → `a04` (`b579816`) corrige topologia mas pré-revisão REJEITA (falta `numberInputs: 7`) → `a05` Codex (`dddb84f`) ✅ pré-revisão Claude PASS (115/115; numberInputs===7 + topologia + hashes a03 preservados + sandbox==workflow). Brief Antigravity rodada 3 entregue. Padrão inegociável consolidado: (a) multi-fan-in pra Code/Function node EXIGE Merge antes; (b) Merge consolidador DEVE declarar `numberInputs` = nº de upstreams. Restam: Antigravity rodada 3 → smoke real (decisivo: 19+0) → publish → T6 fecha. G2/G3 pro Lote 2. |
