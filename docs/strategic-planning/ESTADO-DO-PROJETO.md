...
## 2. Onde estamos AGORA (snapshot 2026-06-14)
...
Lote 2 entregue 2026-06-03 pendente re-smoke A2.3 Aprovado. **Execução de
Demandas** com **Lote 0 Concluído 2026-06-14** (v0.3 em git + 2 ADRs `Aceito`
no Notion + 2 DBs `PHI - Demandas` e `PHI - SOPs` + âncora `[HANDOFF]` +
SOP v1.0 `Vigente`); brief Codex pro Lote 1 (Pacing/verba E2E Crítica)
entregue, aguarda implementação. **Curador** com Lote 0 concluído
estruturalmente 2026-06-04 (Catálogo populado parcialmente — agora **10
artefatos catalogados**, +6 da Execução; backlog de catalogação retroativa
em aberto, ver §5). **Priorização** com L1 **ATIVO em produção desde
2026-06-04**; pendência: timezone do cron. **Comercial** com WF-Deduplicar
em execução real desde 2026-06-05 (manual; 4 débitos legados; sem owner
formal). **Telemetria Mínima** Lote 1 **EM PRODUÇÃO desde 2026-06-13**
(`a05`, smoke verde 19+0); T6 RESOLVIDO. **Documentação e Ferramentas**
...
| Lote 1 (Engine mínimo — Pacing/verba E2E classe Crítica) | **`a03` (`6e49fda`) APROVADO Claude — Antigravity PULADO por decisão Olavo (2026-06-15) — SMOKE REAL autorizado.** Pré-revisão Claude aprovou (build-time injection ADR-19 restaurado, $env zero, cirurgia perfeita com orq/qg hash idêntico ao a02, B1/B3/PHI_EVENTOS/Merge/Gemini preservados). Olavo optou por pular a etapa Antigravity e ir direto ao smoke (smoke é o juiz decisivo — lição da Telemetria). **Ressalva registrada:** sem Antigravity, o smoke é a única rede externa restante; nesta saga o próprio Claude errou B2/B4, então smoke carrega mais peso. **Smoke real autorizado** — 9 passos (brief Antigravity §"Após aprovação"). Pré-req: build com `EXEC_WEBHOOK_KEY` injetado (ADR-19), credenciais Notion+Telegram+Gemini reais, share dos 3 DBs (Demandas/SOPs/Eventos) com integração. Sem smoke verde, não publica | Sem 2-4 |
...
| 2026-06-15 | Smoke real Execução Lote 1 (Pacing E2E) | Olavo executa 9 passos. Pré-req: build com `EXEC_WEBHOOK_KEY` real (ADR-19), credenciais Notion+Telegram+Gemini, share 3 DBs com integração. Smoke verde → palavra final consolidada → publish (active:true) → Lote 1 Concluído. **PAUSADO 2026-06-15:** smoke rodará sobre a arquitetura final após `a04` v2 (refactor HTTP→Notion native). Decisão postergar smoke + refatorar agora (AskUserQuestion 2026-06-15) pra evitar resmoke duplo e publicar versão definitiva. | Pausado até a04 v2 |
| 2026-06-15 | **Codex `a04` v2 Lote 1: refactor HTTP Notion → Notion native v2.2** | Brief reescrito em `docs/handoff/2026-06-15-execucao-lote1-codex-a04-brief.md`. Substituir 9 HTTPs `api.notion.com` (2 intake + 2 orq + 5 qg — 5 POST + 4 PATCH) por `n8n-nodes-base.notion` v2.2 (`databasePage.create` e `databasePage.update`). Code nodes upstream produzem campos avulsos em vez de `demanda_body`/`event_body`. `databaseId` com Resource Locator (`mode: list`, `value: <UUID page_id>`, `cachedResultName: <Nome do DB>`). Buscar SOP/Demandas natives **não tocados**. ADR-19 (EXEC_WEBHOOK_KEY) preservado. EXEC_NOTION_TOKEN não existe nos workflows (grep confirmou) — HTTPs já usavam credencial via predefinedCredentialType. ps1 ganha checks novos (0 HTTP api.notion.com, UUID em databaseId.value, cachedResultName presente, EXEC_NOTION_TOKEN ausente). | Substitui o a04 v1 (band-aid de troca de ID); desbloqueia smoke definitivo |
...
| v0.1.14 | 2026-08-21 | **Frente Webview — Lote 1 (inspeção + base) fechado.** Inspecionados os 2 projetos Lovable de partida: A=*Client Dossier Revival* (`01153f8e…`, TanStack Start, formulário do Dossiê que escreve em localStorage) e B=*PHI Dashboard* (`ff1059aa…`, Vite+React+shadcn, dashboard view-only de campanhas, publicado, hook único `usePhiData` pronto p/ backend). **Decisão: base = Projeto B** — domínio (métricas de campanha) é o núcleo da missão, já é view-only (honra "exibe, não escreve" + ADR-003), stack padrão e mais madura. Migrar do A só a arquitetura de informação do Dossiê (9 seções) como exibição read-only. Registrado em `docs/strategic-planning/webview/` (ADR-webview-001 + CHECKLIST) e `docs/handoff/2026-08-21-webview-lote1-execution-log.md`. **Próximo: W2 merge estrutural — BLOQUEADO até OK de budget do Olavo (send_message Lovable gasta créditos, guardrail §6).** |
| v0.1.15 | 2026-08-22 | **Frente Webview — Lote 2 (merge estrutural) fechado.** Na base Lovable (Projeto B, PHI Dashboard `ff1059aa`) foi consolidada a área "Visão Cliente": Dossiê do Cliente (9 seções) portado do Projeto A como EXIBIÇÃO read-only. Entregues `clientTypes.ts`, `sections.ts`, `clientMock.ts` (6 clientes, com campos vazios propositais p/ N/D), hook `useClientData` (mesmo padrão do `usePhiData`, pronto p/ backend), `DossierField.tsx`, nav "Clientes", e páginas `ClientsList` (`/clientes`) + `ClientDetail` (`/clientes/:client`). Commit Lovable `6c3399478`, 5 créditos (budget aprovado). Guardrails verificados na revisão dos 8 arquivos: view-only (sem input/form/localStorage), NÃO recalcula score (`aggregateClient` é agregação de exibição), N/D honesto, reusa design system, rotas acima do catch-all, Overview/Campanhas intactas. Registrado em `docs/handoff/2026-08-22-webview-lote2-execution-log.md` + CHECKLIST. **Próximo: W3 backend BigQuery (trocar mock→edge functions), requer OK de budget.** |
| v0.1.16 | 2026-08-24 | **Frente Webview — Lote 3 (backend) + pivot para self-hosted VPS.** O build do backend no Lovable travou (>1h); com VPS+hospedagem do Olavo disponíveis, migramos o webview para self-hosted, versionado em git `webview/` (ADR-webview-002). Base trazida do repo phi-dashboard (estava sem o W2) e **W2 reaplicado no git** (7 arquivos + 2 edições). **W3 backend Node/Express** (`webview/server/`): auth service account (JWT RS256→OAuth2), leitura REST do BigQuery (`phi_score_current` + `raw_campaign_data`, partição date), guardrails (conversions=0⇒CPA/ROAS null; erro/ausente⇒null), `/api/phi-snapshot` (com `?debug=1` p/ descobrir colunas) e `/api/phi-score-history`; serve o build estático. Front: `usePhiData`→fetch do backend, erro tratado sem inventar dados; `useClientData` segue mock (W4). Verificação: `npm run build` OK, backend sobe e `/api/health` responde, `/api/phi-snapshot` sem segredo dá 502 tratado. Docs: ADR-002 + GUIA-DEPLOY-VPS + README. **Falta o Olavo:** deploy no VPS + colar `GCP_SA_KEY` + validar KIL (ajustar `COLS` se preciso). Próximo: W4 (Notion). |
| v0.1.17 | 2026-08-24 | **Webview W3 CONCLUÍDO/validado em prod + W4 iniciado (nomes).** Deploy no EasyPanel (Dockerfile, contexto na raiz). Backend lê BigQuery com a SA `antigravity-agent` (billing=dados=`project-0e7c58d4-656f-49e8-807`). Mapeamento real: score=`phi_value`, status=`phi_classification`, client=`client_id`; KPIs derivados dos brutos (cost/clicks/impressions/conversions/revenue), `cpaTarget`=`primary_metric_goal`; guardrail conversions=0⇒CPA/ROAS N/D. Validado com KIL (`GADS-21149189736`, score 59/WARNING). Corrigido crash da página de detalhe (à prova de nulos ⇒ N/D) que causava tela em branco + navegação travada. W4 iniciado: `notion.js` traduz campaign_id→Nome da campanha e CLI-4→Nome do cliente (best-effort via `NOTION_TOKEN`, já inserido). Commits be750b2→76546bc. Próximo: Visão Cliente com dados reais do Notion (dossiê hoje ainda é mock). |
| v0.1.18 | 2026-08-24 | **Webview W5 + W6 concluídos — frente entregue (MVP).** W4 validado em prod (Visão Cliente híbrida: cadastro real do Notion Clientes + dossiê "a preencher"; lista só clientes com campanhas). W5: gráfico Evolução do Score agora usa dado real via `/api/phi-score-history` (lê `phi_score_history`, robusto ao nome da coluna de data=`calculated_date`); KPIs derivados e N/D honesto. W6: navegação cliente⇄campanha (cliente clicável no detalhe da campanha → página do cliente; campanhas clicáveis na página do cliente). Backend Node (`webview/server/`) + front React, deploy EasyPanel. Lotes W1–W6 concluídos. Pendências futuras: preencher o dossiê de marca quando houver fonte; otimizar bundle; auth multi-tenant. |
| v0.1.19 | 2026-08-24 | **Webview W7 — painel operacional da campanha (Notion).** Novo endpoint `/api/campaign-detail?campaign=<id>` que junta por campanha (via page_id/relação e campaign_id texto): Métricas Operacionais 7d(30d) (base Campanhas), Tarefas ativas com link direto p/ abrir no Notion, Histórico de Otimizações (Log de Otimizações), Daily Entry (Observações Diárias), Análise dos agentes (PHI - ANÁLISES: maestro_diagnostico/decisao/proximos_passos, leitura, severidade, flags), e Anúncios com os mesmos KPIs da campanha (já vêm no formato 7d/30d do Notion) + diagnóstico. Backend usa a API de data sources do Notion (2025-09-03) para os filtros por relação; resiliente por seção (Promise.allSettled). Front: CampaignDetail reescrito com todas as seções, N/D honesto. Endpoint auxiliar `/api/notion-debug?db=<id>`. Build validado. Commit 5eea109. Pendente: validar em prod após redeploy (depende de o campaign_id estar preenchido na base Campanhas p/ resolver as relações). |
| v0.1.20 | 2026-08-25 | **Webview W7 — ajustes do painel da campanha (feedback do Olavo).** 5 correções: (1) **CPA mostrava o investido** — os nomes dos campos na base Campanhas são enganosos: `CPA Campanha` guarda o **Investido** e `CPL Campanha` guarda a **métrica-mãe real (ex.: CPA)**. Remapeado no backend (`notion.js` → `ops.investido`/`ops.metricaMaeNome`/`ops.metricaMaeValor` + `metaMae`/`resultadoMae`); card de métrica-mãe agora tem rótulo dinâmico. (2) **Investido só trazia 30d** — resolvido no mesmo remapeamento (formato 7d(30d)). (3) **Daily Entry: só a última análise** (`slice(0,1)`). (4) **Anúncios: arredondar Investido e Conversões** — helper `roundNums()` arredonda todo número para 2 casas nos KPIs dos anúncios e da campanha. (5) **Daily Entry e Anúncios lado a lado** (grid 2 colunas). Front `CampaignDetail.tsx` + tipo `CampaignOps` (`useCampaignDetail.ts`). Build validado. Commit ae8c571. Pendente: redeploy no EasyPanel + validação do Olavo. |
