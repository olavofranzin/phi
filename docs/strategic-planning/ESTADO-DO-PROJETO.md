...
populado parcialmente — 4 artefatos catalogados, backlog de catalogação
retroativa em aberto, ver §5). **Priorização** com L1 **ATIVO em produção
desde 2026-06-04** (n8n `cgw7ozJ7Zk9jBrj1`); pendência aberta: timezone do
cron (UTC vs BRT). **Comercial** com WF-Deduplicar em execução real desde
2026-06-05 (gatilho manual; 4 débitos técnicos legados). **Telemetria
Mínima** Lote 1 **EM PRODUÇÃO desde 2026-06-13** — workflow
`VubalOUaoBteCyC6` (`a05`/`dddb84f`), schedule diário 08:30 BRT, smoke
real verde (19+0 linhas, digest 2×); T6 RESOLVIDO. **Documentação e
Ferramentas** aberta 2026-06-04 (este doc). **Dashboard de produto**
(Lovable) em scoping — aguardando acesso aos protótipos.
...
|---|---|---|
| Lote 0 (Fundação — SOP, modelo, ADRs) | **v0.3 fechada 2026-06-14** (`BRUTO-v0.3-design.md`) — 4 decisões travadas (ciclo de vida, Pacing/verba=tipo Lote 1=Crítica, DB `PHI - SOPs` versionando, `versao_sop_aplicada` na Demanda) + 2 ADRs rascunho em git (Tiering Pro/Flash + Eventos canônicos+sink BQ). Aguarda passos Notion (DBs + âncora + SOP v1.0) | Sem 1-2 |
| Lote 1 (Engine mínimo — Pacing/verba E2E classe Crítica) | Backlog (escopo travado: intake alerta → Orquestrador → SLA → Telegram → quality-gate → digest) | Sem 2-4 |
| Lote 2 (Padronizador / quality-gate como agente Flash dedicado) | Backlog | Sem 4-6 |
...
- **ADR-rascunho — Tiering de Agentes IA (Pro/Flash):** rascunho em git
  (`docs/strategic-planning/execucao-demandas/ADR-rascunho-tiering-agentes-ia.md`),
  criado 2026-06-14. Cristaliza padrão já aplicado de facto em Onboarding.
  Pendente aprovação Olavo → criação no Notion como `Aceito`.
- **ADR-rascunho — Eventos canônicos da Operação Interna + Sink BigQuery futuro:**
  rascunho em git (`docs/strategic-planning/execucao-demandas/ADR-rascunho-eventos-canonicos-sink-bq.md`),
  criado 2026-06-14. Modelo extensível pra todas as áreas. Complementa ADR-010.
  Pendente aprovação Olavo → criação no Notion como `Aceito`.
...
| 2026-06-04 | ~~v0.3 Execução + abertura SOP/DB Notion canônico~~ | ✅ **v0.3 fechada 2026-06-14** (`BRUTO-v0.3-design.md` + 2 ADRs rascunho). Restam passos Notion (criar DBs `PHI - Demandas` + `PHI - SOPs` + âncora `[HANDOFF] Execução de Demandas` + SOP v1.0) + brief Codex pro Lote 1. | Parcial |
| 2026-06-14 | Notion canônico Execução de Demandas | Criar DB `PHI - Demandas` (schema §9 v0.2 + `versao_sop_aplicada`), DB `PHI - SOPs` (schema §4 v0.3), âncora `[HANDOFF] Execução de Demandas`, SOP v1.0 enxuto (página Notion + ponteiro git). Depende de aprovação dos 2 ADRs rascunho. | Bloqueia brief Codex pro Lote 1 |
| 2026-06-14 | ADRs rascunho (Tiering Pro/Flash + Eventos canônicos+sink BQ) → Notion `Aceito` | Olavo revisa em git (`docs/strategic-planning/execucao-demandas/ADR-rascunho-*.md`); aprovação → eu crio páginas no DB `PHI™ Decisões (ADR)` como `Aceito` | Bloqueia Notion canônico Execução |
| 2026-06-14 | Brief Codex Lote 1 Execução (Pacing/verba E2E classe Crítica) | Escrever em `docs/handoff/` após Notion canônico abrir. Escopo travado no §5 do v0.3. | Próxima sessão |
