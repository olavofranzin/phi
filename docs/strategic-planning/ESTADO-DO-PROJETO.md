...
## 2. Onde estamos AGORA (snapshot 2026-06-16)
...
**Execução de Demandas**: Lote 0 Concluído 2026-06-14 (v0.3 + 2 ADRs `Aceito`
+ 2 DBs `PHI - Demandas`/`PHI - SOPs` + âncora `[HANDOFF]` + SOP v1.0
`Vigente`); **Lote 1 piloto `WF-EXEC-QualityGate-Pacing` ATIVO em produção
2026-06-16** após smoke real verde + consolidação `a04-qg-fix-1` (`72b4086`)
fechando drift n8n↔repo (2 demandas E2E: PASS=Entregue, FAIL=Em execucao +
Telegram com checklist; 4 eventos canônicos em `PHI - Eventos`; idempotência
confirmada na 2ª execução). **Intake-Pacing e Orquestrador-Pacing pendentes
refactor `a04-intake`/`a04-orq`** — mesma cirurgia HTTP→Notion native nos
restantes (2 HTTPs em cada). **Curador** Lote 0 concluído estruturalmente
2026-06-04 (Catálogo populado parcialmente; backlog retroativo em aberto,
ver §5). **Priorização** L1 ATIVO em produção desde 2026-06-04 (n8n
...
| Lote 1 (Engine mínimo — Pacing/verba E2E classe Crítica) | **PARCIAL — `WF-EXEC-QualityGate-Pacing` ATIVO em produção (2026-06-16).** `a04-qg-fix-1` (`72b4086`) consolidou 6 fixes inline (Validar DoD lê de `Buscar Demandas Em Revisao`; `utcNow` date-only; 9 expressions × 2 creates via `.all().find` por `demanda_id`; Telegram com fallback `'(sem texto)'`) + 4 cleanups defensivos (`alwaysOutputData:false` nos 2 search nodes; guards `page.id`; remoção `...page`; ps1 com 11 checks novos) após smoke real verde de 2026-06-16 (2 demandas E2E: PASS=Entregue, FAIL=Em execucao com Telegram; 4 eventos em `PHI - Eventos`; idempotência confirmada). **`Intake-Pacing` e `Orquestrador-Pacing` pendentes** — briefs `a04-intake` e `a04-orq` substituem 2 HTTPs Notion em cada por Notion native v2.2 (mesma cirurgia validada no QG). Lote 1 fechado quando os 3 WFs estiverem ativos | Sem 2-4 |
...
| 2026-06-16 | **Brief Codex `a04-intake`: refactor 2 HTTPs Notion no Intake-Pacing** | Substituir `[Exec Intake] Criar Demanda` (POST `/v1/pages` → `databasePage.create` no DB Demandas) e `[Exec Intake] Criar Evento demanda.criada` (POST → `databasePage.create` no DB Eventos) por `n8n-nodes-base.notion` v2.2. Aplicar aprendizados do QG: search nodes sem `alwaysOutputData:true`, evitar pairedItem em consumers downstream, guards `page.id`, expressions via `.all().find()` ou `$json` direto onde aplicável. Brief a escrever. Smoke isolado depois do push. | Próximo |
| 2026-06-16 | **Brief Codex `a04-orq`: refactor 2 HTTPs Notion no Orquestrador-Pacing** | Substituir `[Exec Orq] Atualizar Demanda Priorizada` (PATCH → `databasePage.update`) e `[Exec Orq] Criar Evento demanda.priorizada` (POST → `databasePage.create` em Eventos) por Notion native v2.2. Mesmos aprendizados do QG. Brief a escrever. Smoke isolado depois do push. Lote 1 Execução FECHA quando este último ativo. | Próximo |
