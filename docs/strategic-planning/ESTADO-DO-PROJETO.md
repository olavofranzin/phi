...
| 2026-06-13 | **Auditoria de workflows existentes pelo bug multi-fan-in** — Aprendizado novo confirmou que multi-fan-in pra Code/Function node dispara N execuções silenciosamente | Auditar: 5 WFs Lote 1 Onboarding (A2.1, A2.2, A2.5, A2.9, A2.11) + WF-PRIOR-L1-Abertura-Projeto-Setup + WF-COM-Deduplicar-Leads-HubSpot. Verificar via MCP n8n se algum tem multi-fan-in pra Code node sem Merge consolidador. Se houver, briefar fix. | Não bloqueia hoje; risco latent de dados duplicados ou inconsistentes em prod |
