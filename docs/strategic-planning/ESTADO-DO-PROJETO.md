...
| 2026-06-15 | Smoke real Execução Lote 1 (Pacing E2E) | Olavo executa 9 passos. Pré-req: build com `EXEC_WEBHOOK_KEY` real (ADR-19), credenciais Notion+Telegram+Gemini, share 3 DBs com integração. Smoke verde → palavra final consolidada → publish (active:true) → Lote 1 Concluído. **Em andamento 2026-06-15:** Olavo importou workflows (Validar Secret destravado após orientação Claude), iniciou config dos nodes Notion → identificou discrepância data_source_id × page_id em HTTP requests (`POST /v1/pages`) → fix inline aplicado nos 3 nodes do Intake (Normalizar SOP Vigente, Preparar Demanda e Evento, Montar Evento demanda.criada) trocando `cd1ab757` → `a5c6b6ae-3e9c-4619-a3c3-48e58c75c25b` (Demandas page_id) e `3423df0d` → `c64f600e-4f46-4b2b-ac22-c1e425c8966e` (Eventos page_id). Smoke continua. | Em andamento |
| 2026-06-15 | **Codex `a04` Lote 1: cristalizar troca data_source_id→page_id nos HTTP** | Brief entregue (`docs/handoff/2026-06-15-execucao-lote1-codex-a04-brief.md`). Troca afeta 7 nodes em 3 workflows (3 Intake + 2 Orq + 2 QG). `generate_export.js` ganha constantes `DB_DEMANDAS_PAGE`/`DB_EVENTOS_PAGE` separadas das de Notion nativo. ps1 ganha 2 checks. Notion-Version mantém `2022-06-28` (mudar versão é decisão arquitetural separada). | Não bloqueia smoke (fix inline já cobriu Intake); cristaliza repo + cobre Orq+QG |
