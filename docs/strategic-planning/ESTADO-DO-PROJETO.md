...
| L1.5 Agregador T28 — promoção phi_dev -> phi_prod | **Próximo** | Trocar `phi_dev`->`phi_prod` nos 6 BQ Inserts via MCP `setNodeParameter`. Pré-requisito: DDL `phi_prod.t28_*` aplicado (confirmar antes). Re-smoke real em prod com mesmo CLI-4. |
| L2 Error Handler global (ADR-26) + saneamento dívida arquitetural (deletar cadeia morta Merge1->Calculate KPIs disabled + refatorar Adaptador para conexão explícita) | Backlog (próximo pós-L1.5, D14) | onError + sub-WF `[Global] Error Handler` + DDL `t28_errors`. Substituir `safe()` silencioso por log+propagação. |
