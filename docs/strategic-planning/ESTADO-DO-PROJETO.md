...
## 2. Onde estamos AGORA (snapshot 2026-06-13)
...
com Lote 0 concluído estruturalmente 2026-06-04 (2 DBs + âncora + Catálogo
populado parcialmente — 3 artefatos catalogados, backlog de catalogação
retroativa em aberto, ver §5). **Priorização** com L1 **ATIVO em produção
desde 2026-06-04** (n8n `cgw7ozJ7Zk9jBrj1`, E2E validado 2×); pendência
aberta: timezone do cron (UTC vs BRT). **Comercial** com WF-Deduplicar em
**execução REAL desde 2026-06-05** (DRY_RUN=false após smoke ENT-20, gatilho
manual sem schedule); área formal ainda vestigial — 4 débitos técnicos
legados registrados. **Telemetria Mínima** Lote 1 — `a03` aprovado por
Antigravity rodada 2 (2026-06-11), mas **smoke E2E 2026-06-13 revelou
regressão arquitetural** (133 linhas em vez de 19 — multi-fan-in dispara
Code node 7×); cleanup feito, brief Codex `a04` entregue (adicionar Merge
Pre-Calcular). Aguarda Codex implementar → re-revisão → re-Antigravity →
novo smoke. **Documentação e Ferramentas** aberta 2026-06-04 (este doc).
**Dashboard de produto** (Lovable) em scoping — aguardando acesso aos
protótipos.
...
| Lote 1 (workflow WF-DOC-Telemetria-Diaria — Onboarding+Curador+Global) | **REGRESSÃO ARQUITETURAL detectada no smoke E2E (2026-06-13).** Workflow `VubalOUaoBteCyC6` rodou no n8n prod (`active: false`): 1ª execução criou **133 linhas** (esperado 19; é exatamente 7×19); 2ª execução criou outras **133** (esperado 0). Causa-raiz: 7 leituras Notion conectam todas em `Calcular Metricas in[0]` — n8n dispara Code node 1× por upstream trigger mesmo em `runOnceForAllItems`. Idempotência também falha porque `$('Buscar Snapshots Existentes').all()` pode estar vazio em iterações triggadas por outros upstreams. Nem pré-revisão Claude (119/119) nem Antigravity (APROVADO rodada 2) detectaram — bug não-estrutural, só smoke runtime captura. Cleanup: 266 linhas apagadas manualmente por Olavo. Workflow `active: false` confirmado. **Brief Codex `a04` entregue 2026-06-13** (`docs/handoff/2026-06-13-telemetria-lote1-codex-a04-brief.md`): adiciona `[Telemetria] Merge Pre-Calcular` (mode append) antes de Calcular Metricas, recebendo as 7 leituras em inputs 0..6 e emitindo 1 batch único → Code node executa 1× só. **17 nodes total**, ps1 atualizada. Aprendizado novo criado (Status `Em análise`): "Multi-fan-in para Code node em n8n dispara N execuções". Aguarda Codex implementar `a04` → pré-revisão Claude → Antigravity rodada 3 → smoke real → publish. |
...
| 2026-06-05/13 | Telemetria Mínima Lote 1 — ciclo de correções | ✅ `a01` (`b15e8dd`) → G1 fix (`327872d`) → brief Antigravity (`ed3d4be`) → REJEITADO rodada 1 → análise factual → brief `a03` → Codex `a03` (`6a9c745`) → pré-revisão Claude 119/119 → brief rodada 2 → ✅ APROVADO Antigravity rodada 2 → **smoke E2E 2026-06-13 REVELOU REGRESSÃO arquitetural** (multi-fan-in dispara Code node 7×; 133 linhas em vez de 19; idempotência zero). Cleanup feito. Workflow `active: false` confirmado. Brief Codex `a04` entregue (`docs/handoff/2026-06-13-telemetria-lote1-codex-a04-brief.md`). Aprendizado novo criado (Status `Em análise`). | Codex implementa `a04` → pré-revisão Claude → Antigravity rodada 3 → novo smoke real → publish |
...
| T6 | **Métricas operacionais não rastreadas** — Onboarding em prod há 9 dias, zero visibilidade | Conversa 2026-06-04 | **Em mitigação — smoke revelou regressão arquitetural** | Ciclo até hoje: Lote 0 (2026-06-04) → `a01` Codex (`b15e8dd`) → G1 fix Claude (`327872d`) → REJEITADO rodada 1 → análise factual → `a03` Codex (`6a9c745`) → pré-revisão Claude 119/119 PASS → ✅ APROVADO Antigravity rodada 2 (2026-06-11). **Smoke E2E 2026-06-13: REGRESSÃO** — workflow rodou sem erro mas criou 133 linhas em vez de 19 na 1ª execução e outras 133 em vez de 0 na 2ª. Causa-raiz: multi-fan-in das 7 leituras pra Code node dispara jsCode 7× (n8n behavior). Nem ps1 (estrutural) nem Antigravity (nó-a-nó) detectam. Cleanup OK (266 linhas apagadas). Brief Codex `a04` entregue (adicionar Merge Pre-Calcular). Padrão inegociável NOVO: multi-fan-in pra Code/Function node EXIGE Merge antes. Restam: Codex `a04` → re-revisão → re-Antigravity → novo smoke → publish → T6 fecha. G2/G3 continuam pro Lote 2. |
