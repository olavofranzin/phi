-- ============================================================================
-- DDL Agregador T28 — 6 tabelas + 3 VIEWs (D-1, D-3)
-- ============================================================================
...
--
-- Política:
--   1. Rodar em phi_dev primeiro (sandbox)
--   2. Smoke real: 1 cliente × 1 semana, comparar t28_campaign vs raw_campaign_data
--   3. Promover pra phi_prod via re-import do workflow com BQ_DATASET=phi_prod
--
-- Convenções:
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_campaign` (
  -- chaves
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_adset` (
  client_id                STRING NOT NULL,
  campaign_id              STRING NOT NULL,
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_ga4_landing` (
  client_id                STRING NOT NULL,
  business_date            DATE   NOT NULL,
  janela                   STRING NOT NULL,
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_gbp_daily` (
  client_id                STRING NOT NULL,
  business_date            DATE   NOT NULL,
  janela                   STRING NOT NULL,
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_clarity_daily` (
  client_id                STRING NOT NULL,
  business_date            DATE   NOT NULL,
  janela                   STRING NOT NULL,
...
-- =============================================
CREATE TABLE IF NOT EXISTS `phi_dev.t28_meta_campaign` (
  client_id                STRING NOT NULL,
...
CREATE OR REPLACE VIEW `phi_dev.t28_campaign_d1` AS
SELECT *
FROM `phi_dev.t28_campaign`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY);

CREATE OR REPLACE VIEW `phi_dev.t28_campaign_d3` AS
SELECT *
FROM `phi_dev.t28_campaign`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 3 DAY);

CREATE OR REPLACE VIEW `phi_dev.t28_adset_d1` AS
SELECT *
FROM `phi_dev.t28_adset`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY);

CREATE OR REPLACE VIEW `phi_dev.t28_adset_d3` AS
SELECT *
FROM `phi_dev.t28_adset`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 3 DAY);

CREATE OR REPLACE VIEW `phi_dev.t28_meta_campaign_d1` AS
SELECT *
FROM `phi_dev.t28_meta_campaign`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY);

CREATE OR REPLACE VIEW `phi_dev.t28_meta_campaign_d3` AS
SELECT *
FROM `phi_dev.t28_meta_campaign`
WHERE janela = 'D-7'
  AND business_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 3 DAY);
...
-- ============================================================================
-- Promoção phi_dev → phi_prod
-- ============================================================================
-- Quando Lote 1 (smoke em phi_dev) ficar verde:
--   1. Rodar este script em phi_prod (substituir "phi_dev." por "phi_prod.")
--   2. Re-importar o workflow do agregador com BQ_DATASET=phi_prod
--   3. Smoke novamente em prod
--
-- Alternativa: bq cp phi_dev.t28_campaign phi_prod.t28_campaign (copia estrutura
-- + dados; usar com cuidado pra não duplicar histórico de phi_dev em prod).
