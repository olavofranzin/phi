...
| **D3** | Quem é o dono de `client_config`? | 🔴 **Reaberta pelo as-built.** O achado A10 mudou a pergunta: **já existem dois writers**, em ambientes diferentes, na mesma coluna | **Nenhuma ação antes de resolver isto.** O `PHI - Subworkflow Campanhas` escreve `primary_metric_type` em `phi_prod`; o workflow `client_config` escreveria `'ROAS'` fixo. **Repontar sem tratar cria conflito M1 e quebra o KIL.** Precisa de ADR próprio |
