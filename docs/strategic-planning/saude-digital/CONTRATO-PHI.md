...
| `phi_score_history` | **`PHI - Pipeline_v2`**, nó `Calcular e Persistir PHI Score` (MERGE por `client_id`+`platform`+`campaign_id`+`calculated_date`) | 07h | `phi_score_current` · Notion · T28 | ✅ **writer único** |
...
| **D3** | Quem é o dono de `client_config`? | 🔴 **Reaberta pelo as-built.** O achado A10 mudou a pergunta: **já existem dois writers**, em ambientes diferentes, na mesma coluna | **Nenhuma ação antes de resolver isto.** ✅ **ADR-39 aberto em 20/09** (`adr-rascunhos/ADR-39-dono-unico-client-config.md`) — propõe o workflow `client_config` como dono único em `phi_prod`, com ordem obrigatória de 5 passos e 6 critérios de aceite. **Aguarda OK do Olavo.** Bloqueia as Fases 2 e 3 do ADR-37 |
